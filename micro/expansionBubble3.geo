 ...
 * */
b2        = 0.05;
xInit     = (xInit+len+pert+height/2.0) + height/1.5;
yInit     = 0.7;
height    = 0.6;
len       = 1.5*height;
pert      = (0.0/100)*(height/2.0);
...
Point(20) = {xInit+len+pert+height/2.0, yInit+height/2.0, 0, b2};
...
pert      = (0.0/100)*(height/2.0);
p1        = 21; 
Point(21) = {xInit, yInit, 0, b3}; // init
Point(22) = {xInit, yInit+height, 0, b3}; 
Point(23) = {xInit+len, yInit+height, 0, b3}; 
Point(24) = {xInit+len, yInit, 0, b3}; 
Point(25) = {xInit+len, yInit+height/2.0, 0, b3}; // center
Point(26) = {xInit+len+pert+height/2.0, yInit+height/2.0, 0, b3};

Line(19) = {p1+2,p1+1};
Line(20) = {p1+1, p1};
Line(21) = {p1, p1+3};
Ellipse(22) = {p1+3, p1+4, p1+4, p1+5};
Ellipse(23) = {p1+5, p1+4, p1+4, p1+2};

Physical Line('wall') = {8, 1, 2, 3, 4, 5, 6, 7};
Physical Line('bubble1') = {9, 10, 11, 12, 13};
Physical Line('bubble2') = {14, 15, 16, 17, 18};
Physical Line('bubble3') = {19, 20, 21, 22, 23};
